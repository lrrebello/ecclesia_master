...
class ReceiptPDF(FPDF):
    def header(self):
        # O cabeçalho será tratado manualmente na função para seguir o layout exato
        pass

    def footer(self):
        # O rodapé será tratado manualmente na função
        pass
...
def generate_receipt(transaction):
    ...
    # Quebra o endereço se for longo
    ...
    # --- TABELA DE DADOS ---
    pdf.set_fill_color(220, 235, 245) # Azul claro do modelo
    pdf.set_font('Arial', 'B', 10)
    ...
    pdf.set_font('Arial', '', 10)
    text = f"Recebemos de V. Exas. pelo meio acima indicado a quantia de {transaction.amount:.2f} {currency} como donativo destinado ao desenvolvimento da atividade desta instituição, concedido sem quaisquer contrapartidas."
    ...
    # No seu ambiente local: /home/lucas/Projetos/ecclesia_master_final/app/static/uploads/receipts/
    # Usando current_app.config para flexibilidade
    upload_folder = current_app.config.get('UPLOAD_FOLDER', 'app/static/uploads')
    
    # Se o caminho for relativo (como em alguns ambientes), tornar absoluto em relação ao app
